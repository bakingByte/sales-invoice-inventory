myApp.controller('courseController',['$scope', '$sce', '$routeParams', 'Course', 'CourseDetail', 'CourseReview', 'Profile', function($scope, $sce, $routeParams, Course, CourseDetail, CourseReview, Profile){
  $scope.course = {};
  $scope.courseDetails = [];
  $scope.courseReviews = [];
  $scope.newReview = {};
  ...
  $scope.course = $scope.courses[$scope.courseId - 1];

  // $scope.getCourse = function() {
  //   Course
  //     .find({id:$scope.courseId})
  //     .$promise
  //     .then(function(course) {
  //       $scope.course = course[$scope.courseId];
  ...
  function getCourseDetails() {
    Course.courseDetails({id:$routeParams.id})
      .$promise
  ...
  function getCourseReviews() {
    CourseReview
      .find({id:$routeParams.id})
      .$promise
      .then(function(courseReviews) {
        $scope.courseReviews = courseReviews;
  ...
  $scope.emailForShareSubject = "Recommended Podcasts for you!!";
  $scope.pageTittle = 'Course';
  ...
  $scope.selected=null;
  $scope.Description='Lorem ipsum represents a long-held tradition for designers, typographers and the like. Some people hate it and argue for its demise, but others ignore the hate as they create awesome tools to help create filler text for everyone from bacon lovers to Charlie Sheen fans.';
  $scope.Details =[
    {
      Id:'1',
      Name:'Angular Js Video Tutorial',
      Type:'Video',
      ReleasedDate:'02-Jan-2016',
      Popularity:'40',
      Path:'https://www.youtube.com/embed/ejBkOjEG6F0',
      OnlinePath:'',
      OfflinePath:''
    },
    {
      ...
      ReleasedDate:'02-Jan-2016',
      Popularity:'70',
      Path: 'https://ia801506.us.archive.org/24/items/captainkidd_1602_librivox/captainwilliam_09_abbott_64kb.mp3',
      OnlinePath:'',
      OfflinePath:''
    },
    {
      ...
      ReleasedDate:'02-Jan-2016',
      Popularity:'20',
      Path:'http://fastandfluid.com/publicdownloads/AngularJSIn60MinutesIsh_DanWahlin_May2013.pdf',
      OnlinePath:'',
      OfflinePath:''
    }
  ];
...
$scope.playButtonClicked=function(param){
  ...
  $scope.sendMail = function() {
    // window.open('mailto:' + $scope.emailForShare + '?subject=' + $scope.emailForShareSubject + '&body=I thought you might find this information interesting:%20%0d%0a'+window.location);
    // $scope.emailForShare = "";
  };

  $scope.subscribeClick = function(course){
    Profile.create();
  };

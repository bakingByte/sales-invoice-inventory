myApp.controller('unplayedController', ['$scope', '$sce', 'Profile','CourseDetail','Course', function($scope, $sce, Profile,CourseDetail,Course) {
  $scope.courseclk= null;
  $scope.SelectedCourse = null;
  $scope.pageTittle="Unplayed";
  $scope.selected = null;
  $scope.UnplayedCourses=[];
  $scope.profile = {};
  $scope.courseDetails = [];
  ...
  function getProfile() {
    Profile
      ...
      .then(function(profile) {
        ...
        for(var i=0;i<$scope.profile.unplayedCourses.length;i++){
          $scope.UnplayedCourses.push($scope.courses[$scope.profile.unplayedCourses[i]]);
        }
        $scope.load();
        getCourseDetails($scope.UnplayedCourses[0].id);
      });
  }
  ...
  $scope.load = function() {
    $scope.SelectedCourse = $scope.UnplayedCourses[0];
  };
